            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg ">
                <div
                    ...
                    <div
                        ...
                        {{-- add button --}}
                        <a href="{{ route('bonCommandes.bonRetours.create', $bonCommande) }}"
                        ...
                        <tbody>
                            @foreach ($bonRetours as $bonRetour)
                                <tr
                                    ...
                                    @if ($bonRetour->STATUT_RETOUR == 'resolue') text-green-600 dark:text-green-400 
                                    @else text-red-600 dark:text-red-400 @endif">
                                    {{-- actions --}}
                                    <td class="px-2 py-3 flex items-center justify-end border-r border-gray-700">
                                        <button id="{{ $bonRetour->id }}-button"
                                        ...
                                        <div id="{{ $bonRetour->id }}"
                                            ...
                                            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
                                                <li>
                                                    <a href="{{ route('bonRetours.edit', $bonRetour->id) }}"
                                    ...
                                    <th class="px-4 w-auto py-3"> {{ $bonRetour->id }} </th>
                                    <td class="px-4 w-auto py-3"> {{ $bonRetour->date_retour }} </td>
                                    <td class="px-4 w-auto py-3"> {{ $bonRetour->motif }} </td>
                                    <td class="px-4 w-auto py-3"> {{ $bonRetour->STATUT_RETOUR }} </td>
                                </tr>
                            @endforeach
                ...
                <div class="p-4">
                    ...
                    {{ $bonRetours->links('vendor.livewire.tailwind') }}
                </div>
